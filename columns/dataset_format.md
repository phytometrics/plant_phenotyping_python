...
画像ファイル名とラベルが紐づいたアノテーションファイル（例えばcsv）を用意することもありますが、クラスごとにフォルダを作成し、その中に画像を配置すれば自明であり、上のワンホットラベルが逐次生成できるため、省略されることも多いです。

## COCOフォーマット

COCO（Common Objects in Context）フォーマットは、コンピュータビジョンの分野で使用される画像データセットのアノテーション形式の一つで、特に物体検出、セグメンテーション、キャプショニングのタスクに広く利用されています。

例えば以下のようなデータセット構造を持ちます。
...
下記はimage1.jpgに紐づく情報です。

１オブジェクトにつき、一行の情報が記述されます。
